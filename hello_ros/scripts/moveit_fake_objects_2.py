...
def move_group_python_interface_tutorial():
  ...
  '''
  fire_hydrant_names = ["fire_hydrant", "fire_hydrant_0"]

  model_states = rospy.wait_for_message("gazebo/model_states", ModelStates, 10)
  fire_hydrant_i = [
      i for i, name in enumerate(model_states.name)
      if name in fire_hydrant_names
  ]
  fire_hydrant_poses = [model_states.pose[i] for i in fire_hydrant_i]

  p = geometry_msgs.msg.PoseStamped()
  p.header.frame_id = robot.get_planning_frame()
  p.pose = fire_hydrant_poses[0]
  scene.add_box("table", p, (0.5, 0.5, 1.4))
  p.pose = fire_hydrant_poses[1]
